...
For each chunk:
1. Run review agents (22 agents in 7-7-8 batches)
...
2. **Spawn 22 review agents in batches (7-7-8):**
   ...
   | 3 | 15-21 + 27: Resource Management, Assertion Quality, Error Handling, Duplicate Code, Dead Code, Over-Engineering, Security Basics, API Surface |

   **Agent prompt:**
   ```
   Read C:\Projects\hattrick\Docs\CODE_REVIEW_AGENTS\{XX_NAME}.md
...
After all 22 review agents complete, spawn two post-review agents **sequentially**:
...
- Launch review agents in batches (7-7-8), never all 22 at once
- Track ALL issues via TaskCreate
- **All issues found MUST be fixed — regardless of whether they appear to be pre-existing**
